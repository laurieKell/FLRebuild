...
setMethod("rebuild", signature(object = "FLBRP"),
  function(object, targetF = computeRefpts(object)["msy","harvest"] * 0,
    ...
    btar = FLQuant(rep(targetSSB, each = dim(fbar(eql))[2]),
                   dimnames = dimnames(propagate(apply(ssb(eql),2,sum), nits)))
...
rebuild2<-function(object, 
            ...
            stk =rebuild::ffwd2(stk, f=fbar(eql)[,-1], sr=eql)
...
rebuildTimeFLBRP<-function(object, 
  ...
  if (dims(stk)["min"]==0)
     rtn=rebuild::tryIt(FLCore:::ffwd(stk, fbar=fbar(object)[,-1]%=%0, sr=object))
  else
    rtn=rebuild::tryIt(ffwd(stk, f=fbar(object)[,-seq(dims(stk)[["min"]]+1)]%=%0, sr=object))
